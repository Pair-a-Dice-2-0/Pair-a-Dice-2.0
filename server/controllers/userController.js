...
userController.addUser = (req, res, next) => {
  ...
  bcrypt.hash(password, saltRounds, function (err, hash) {
    ...
    const queryParams = [username, hash];
    //execute query
...
userController.verifyUser = (req, res, next) => {
  ...
  const queryText = `SELECT _id, username, password, sessioncount FROM users WHERE username = $1`;
  ...
  db.query(queryText, queryParams)
    .then((result) => {
      if (result.rows.length === 0) {
        ...
        const retrievedHash = result.rows[0]['password'];
        console.log(retrievedHash);
        bcrypt.compare(password, retrievedHash, function (err, data) {
          if (data === true) {
            res.locals.user = result.rows[0];
...
userController.sessionFeedback = (req, res, next) => {
  ...
  const queryParams = [
    req.query.partnera_id,
    req.query.partnerb_id,
    req.query.partnera_feedback,
    req.query.partnerb_feedback,
  ];
  db.query(queryText, queryParams)
    .then((result) => {
